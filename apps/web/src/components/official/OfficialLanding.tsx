          ...
          <nav className="mx-auto flex max-w-[1360px] gap-2 overflow-x-auto px-4 pb-2.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {navItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={() => setActiveNav(item.href.replace('#', ''))}
                className={`shrink-0 rounded-lg px-3 py-2 text-sm transition-all ${activeNav === item.href.replace('#', '') ? 'bg-cyan-500/16 text-cyan-100 shadow-[0_0_0_1px_rgba(103,232,249,0.2)]' : 'text-slate-300 hover:bg-cyan-500/10 hover:text-white'}`}
              >
                {item.label}
              </a>
            ))}
