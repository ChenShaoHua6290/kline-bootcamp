  ...
  return (
    <Card className="shrink-0 p-3">
      <div className="mb-2 flex items-center justify-between text-[12px] text-slate-300">
        ...
        <Badge tone="info" className="text-[10px]">{closedTrades.length} 笔</Badge>
      </div>
      <div className="mb-2 grid grid-cols-2 gap-2">
        <div className="text-slate-300">
          <span className="text-[11px] text-slate-400">总交易次数:</span> <span className="text-[12px] font-semibold text-slate-100">{closedTrades.length}</span>
        </div>
        <div className="text-right text-[13px] font-semibold text-rose-300">{winRate.toFixed(2)}%</div>
        <div className="text-slate-300">
          <span className="text-[11px] text-slate-400">盈利次数:</span> <span className="text-[12px] font-semibold text-emerald-300">{wins}</span>
        </div>
        <div className="text-right text-slate-300">
          <span className="text-[11px] text-slate-400">亏损次数:</span> <span className="text-[12px] font-semibold text-rose-300">{losses}</span>
        </div>
      </div>
