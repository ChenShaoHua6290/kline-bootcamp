export function ReviewStatsPanel({ stats }: { stats: any }) {
  const row = (label: string, value: string | number) => (
    <div className="flex items-center justify-between rounded-lg border border-slate-700/70 bg-slate-900/50 px-3 py-2.5">
      <span className="text-[12px] text-slate-400">{label}</span>
      <span className="text-[14px] font-semibold text-slate-100">{value}</span>
    </div>
  );

  return (
    <div className="space-y-2">
      <h3 className="text-[15px] font-semibold text-cyan-300">行为统计</h3>
