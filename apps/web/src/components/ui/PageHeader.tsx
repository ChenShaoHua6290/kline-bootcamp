...
export function PageHeader({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('mb-3 flex flex-wrap items-end justify-between gap-3', className)} {...props} />;
}

export function PageTitle({ className, ...props }: HTMLAttributes<HTMLHeadingElement>) {
  return <h1 className={cn('text-xl font-semibold tracking-[0.01em] text-slate-100 sm:text-2xl', className)} {...props} />;
}
