...
export function AccountPanel({ session, showCurrentPnl = true }: { session: NonNullable<Session>; showCurrentPnl?: boolean }) {
  ...
  return (
    <section className="shrink-0 space-y-1.5">
      <Card className="p-3">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-slate-300">
            <span className="text-[10px] text-slate-400">账户总积分</span>
          ...
          <div className="flex items-center justify-between text-slate-300">
            <span className="text-[10px] text-slate-400">持仓方向</span>
            <span className="text-[11px] font-semibold text-slate-100">{sideText}</span>
          </div>
          <div className="flex items-center justify-between text-slate-300">
            <span className="text-[10px] text-slate-400">持仓数量</span>
            <span className="text-[11px] font-semibold text-slate-100">{qty} 股</span>
          </div>
          <div className="flex items-center justify-between text-slate-300">
            <span className="text-[10px] text-slate-400">总盈亏</span>
      ...
      {showCurrentPnl ? (
        <Card className="p-3">
          <div className="mb-1.5 text-[13px] font-semibold text-slate-100">当前盈亏</div>
      ...
      <div className="rounded-lg border border-emerald-500/25 bg-slate-900/70 p-2.5">
        <div className="mb-1 text-[13px] font-semibold text-slate-100">持仓收益</div>
        <div className="grid grid-cols-3 gap-2 text-[10px]">
          <div>
            <div className="text-[10px] text-slate-400">成本</div>
            <div className="text-[11px] font-semibold text-slate-100">¥{session.position ? session.position.entryPrice.toFixed(2) : '--'}</div>
          </div>
          <div>
            <div className="text-[10px] text-slate-400">浮动盈亏</div>
            <div className={`text-[11px] font-semibold ${floatingPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{floatingPnl.toFixed(2)}</div>
          </div>
          <div>
            <div className="text-[10px] text-slate-400">收益率</div>
            <div className={`text-[11px] font-semibold ${holdingReturnPct >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              {holdingReturnPct >= 0 ? '+' : ''}
