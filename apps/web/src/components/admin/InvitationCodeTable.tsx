...
export function InvitationCodeTable({
  ...
  const desktopCols = 'grid-cols-[1.15fr_0.65fr_0.6fr_0.65fr_1fr_1fr_1fr]';
  return (
    <>
        ...
        {rows.map((row) => (
          <div key={row.id} className="ui-card p-3 text-xs text-slate-200">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <Button size="sm" variant="outline" disabled={pendingToggleId === row.id || pendingDeleteId === row.id} onClick={() => onToggleActive(row)}>
      ...
      <TableWrap className="hidden overflow-x-hidden md:block">
      <Table>
        <thead>
          <tr className={`grid ${desktopCols} gap-2.5 border-b border-slate-700/70 px-3 py-2 text-[11px] text-slate-400`}>
            <th className="text-left font-medium">邀请码</th>
            <th className="text-right font-medium">最大次数</th>
            <th className="text-right font-medium">已使用</th>
            <th className="text-center font-medium">状态</th>
            <th className="text-left font-medium">过期时间</th>
            <th className="text-left font-medium">创建时间</th>
      ...
      <div className="max-h-[64vh] overflow-y-auto">
        {rows.map((row) => (
          <div key={row.id} className={`grid ${desktopCols} items-center gap-2.5 border-b border-slate-800/80 px-3 py-2.5 text-xs text-slate-200 transition hover:bg-slate-800/45`}>
            <div className="truncate font-mono">{row.code}</div>
            <div className="text-right tabular-nums">{row.maxUses}</div>
            <div className="text-right tabular-nums">{row.usedCount}</div>
            <div className="flex items-center justify-center">
              <Badge tone={row.isActive ? 'success' : 'default'}>{row.isActive ? '启用' : '停用'}</Badge>
            </div>
            <div className="truncate tabular-nums">{row.expiresAt ? new Date(row.expiresAt).toLocaleString('zh-CN') : '--'}</div>
            <div className="truncate tabular-nums">{new Date(row.createdAt).toLocaleString('zh-CN')}</div>
