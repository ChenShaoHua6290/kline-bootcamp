...
describe('TrainingService.getBarsWindow', () => {
  it('clamps query upper bound to currentTimePointer (safeTo)', async () => {
    ...
    marketDataService.getBarsByTimeRange.mockResolvedValue([
    ...
    expect(marketDataService.getBarsByTimeRange).toHaveBeenCalledTimes(1);
    const calledToTs = marketDataService.getBarsByTimeRange.mock.calls[0][4];
    expect(new Date(calledToTs).toISOString()).toBe('2026-01-01T00:30:00.000Z');
